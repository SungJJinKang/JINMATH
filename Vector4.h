...
namespace math
{
	...
	struct Vector<4, T>
	{
		...
		FORCE_INLINE Vector(T xValue, T yValue, T zValue, T wValue) noexcept
		...
		FORCE_INLINE Vector(const Vector<3, T>& vector, T _w = 0) noexcept
			: x{ vector.x }, y{ vector.y }, z{ vector.z }, w{ _w }
		{
		}

		template <typename X>
		FORCE_INLINE Vector(const Vector<3, X>& vector, T w = 0) noexcept
	...
	template<typename T>
	FORCE_INLINE Vector<4, T> operator+(const Vector<4, T>& vector) noexcept
	{
		return vector;
	}
...
namespace math
{
	...
	extern template struct math::Vector<4, FLOAT32>;
}
