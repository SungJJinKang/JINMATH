...
namespace math
{
	...
	struct Matrix<4, 4, T>
	{
		...
		template <typename X>
		[[nodiscard]] inline constexpr Vector<4, X> operator*(const Vector<3, X>& vector) const noexcept
		{
			return Vector<4, X>
			{
	...
	template <typename T>
	FORCE_INLINE constexpr Matrix<4, 4, T> operator+(const Matrix<4, 4, T>& matrix) noexcept
	{
		return matrix;
	}
	...
	/// </summary>
	template <typename T>
	inline constexpr void ExtractPlanesFromVIewProjectionMatrix(const Matrix<4, 4, T>& viewProjectionMatrix, math::Vector<4, T>* sixPlanes, bool normalize) noexcept
	{
