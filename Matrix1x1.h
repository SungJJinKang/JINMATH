...
namespace math
{
	...
	struct Matrix<1, 1, T>
	{
		...
		FORCE_INLINE Matrix(const col_type& column0Value) noexcept
			: columns{ column0Value }
		{
		}

		template <typename X>
		FORCE_INLINE Matrix(const col_type_template<X>& column0) noexcept
			: columns{ column0 }
		{
		}
		...
		template <typename X>
		FORCE_INLINE type operator+(const Matrix<1, 1, X>& rhs) const noexcept
		{
		...
		*/
		...
		template <typename X>
		FORCE_INLINE type& operator+=(const Matrix<1, 1, X>& rhs) noexcept
		{
	...
	template<typename T>
	FORCE_INLINE Matrix<1, 1, T> operator+(const Matrix<1, 1, T>& matrix) noexcept
	{
		return matrix;
	}
